...
function Yetii(){
	...
	this.next=function(){var a=(this.defaults.active+1>this.tabs.length)?1:this.defaults.active+1;
	this.show(a);
	this.defaults.active=a};
	this.previous=function(){var a=((this.defaults.active-1)==0)?this.tabs.length:this.defaults.active-1;
	this.show(a);
	this.defaults.active=a};
	...
	this.gup=function(a){a=a.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");
	var b="[\\?&]"+a+"=([^&#]*)";
	var c=new RegExp(b);
	...
	for(var i=0;i<this.links.length;i++){
		...
		this.links[i].onclick=function(){
			...
			if(f.defaults.wait)
				f.timer2=setTimeout(function(){f.rotate(f.defaults.interval)},f.defaults.wait*1000);
			return false
		}
	}
